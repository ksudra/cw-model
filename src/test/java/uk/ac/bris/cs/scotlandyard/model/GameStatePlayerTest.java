...
public class GameStatePlayerTest extends ParameterisedModelTestBase {
	...
	@Test public void testDetectiveNotPickingAMoveDoesNotAffectMrX() {
		...
		GameState state = gameStateFactory.build(standard24RoundSetup(), black, red, blue);
	...
	@Test public void testMrXMovesToDestinationAfterDoubleMove() {
		...
		GameState state = gameStateFactory.build(
				new GameSetup(standardGraph(), rounds(true, true, true)), mrX, blue);
	...
	@Test public void testMrXMovesToDestinationAfterTicketMove() {
		...
		GameState state = gameStateFactory.build(
				new GameSetup(standardGraph(), rounds(true, true)), mrX, blue);
	...
	@Test public void testDetectiveLocationHoldsAfterPassMove() {
		...
		GameState state = gameStateFactory.build(standard24RoundSetup(), mrX, red, blue);
	...
	@Test public void testDetectiveLocationAlwaysCorrect() {
		...
		GameState state = gameStateFactory.build(
				new GameSetup(standardGraph(), rounds(true, false, true)), mrX, blue);
	...
	@Test public void testMrXTravelLogCorrectRevealRound() {
		var mrX = new Player(MRX, defaultMrXTickets(), 45);
		var blue = new Player(BLUE, defaultDetectiveTickets(), 94);
		...
		// Mr X's location should be available after every reveal round, but not at the start of
		// the game
	...
	@Test public void testMrXTravelLogCorrectOnHiddenRound() {
		var mrX = new Player(MRX, defaultMrXTickets(), 45);
		var blue = new Player(BLUE, defaultDetectiveTickets(), 94);

		GameState state = gameStateFactory.build(
				new GameSetup(standardGraph(), rounds(true, false, false, true)), mrX, blue);
...
//						LogEntry.reveal(TAXI, 79));
	...
	@Test public void testMrXTravelLogCorrectWithOneRevealRound() {
		var mrX = new Player(MRX, defaultMrXTickets(), 45);
		var blue = new Player(BLUE, defaultDetectiveTickets(), 94);
		GameState state = gameStateFactory.build(
				new GameSetup(standardGraph(), rounds(true)), mrX, blue);
		state = state.advance(taxi(MRX, 45, 46));
		state = state.advance(taxi(BLUE, 94, 93));
		assertThat(state.getMrXTravelLog()).containsExactly(LogEntry.reveal(TAXI, 46));
	}

	@Test public void testMrXTravelLogCorrectWithOneHiddenRound() {
		var mrX = new Player(MRX, defaultMrXTickets(), 45);
		var blue = new Player(BLUE, defaultDetectiveTickets(), 94);
		GameState state = gameStateFactory.build(
				new GameSetup(standardGraph(), rounds(false)), mrX, blue);
		state = state.advance(taxi(MRX, 45, 46));
		state = state.advance(taxi(BLUE, 94, 93));
